...
public class Quotation_PI extends Base {
	...
	public static WebElement AnnualIncome(WebDriver driver) {
		WebDriverWait wait = new WebDriverWait(driver, 80);
		WebElement user = wait.until(ExpectedConditions.visibilityOfElementLocated(getlocators("AnnualIncome")));
		return user;
	}
	
	public static WebElement TaxRate(WebDriver driver) {
		WebDriverWait wait = new WebDriverWait(driver, 80);
		WebElement user = wait.until(ExpectedConditions.visibilityOfElementLocated(getlocators("TaxRate")));
		return user;
	}
